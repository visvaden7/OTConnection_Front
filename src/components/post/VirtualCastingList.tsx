    ...
    <div className="virtual-casting-container">
      ...
      {virtualCastingData.map((item, idx) => (
        <Link to={`/community/virtual-casting/${item.post_id}`} key={idx}>
          <div className="virtual-casting-card" style={{ backgroundColor: "lightgrey", width: "80%", marginBottom: "20px", padding: "20px" }}>
            <div className="virtual-casting-header">
              <h2>{item.title} 가상캐스팅</h2>
            </div>
            <div className="virtual-casting-body" style={{ display: "flex", flexDirection: "row", gap: "20px" }}>
              {/* 메인 캐릭터 이미지 및 정보 */}
              ...
              {/* 서브 캐릭터 이미지 및 정보 */}
              <div className="casting-details" style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                <div className="character-card">
                  <img width="150" height="200" src={item.char_main_url} alt={`${item.char_main} 캐릭터`} />
                  <p>{item.char_main}</p>
                </div>
                <div className="character-card">
                  <img width="150" height="200" src={item.char_sub1_url} alt={`${item.char_sub1} 캐릭터`} />
                  <p>{item.char_sub1}</p>
                </div>
                <div className="character-card">
                  <img width="150" height="200" src={item.char_sub2_url} alt={`${item.char_sub2} 캐릭터`} />
                  <p>{item.char_sub2}</p>
                </div>
                <div className="character-card">
                  <img width="150" height="200" src={item.char_sub3_url} alt={`${item.char_sub3} 캐릭터`} />
                  <p>{item.char_sub3}</p>
                </div>
              </div>
            </div>
